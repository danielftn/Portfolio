...
ProjectProps) {
    ...
    return (
        <motion.div
            ...
            overflow-hidden sm:pr-8 relative sm:h-[24rem] mb-3 sm:mb-8 last:mb:0 group-even:pl-8
            hover:bg-gray-200 transition'>
                <div className='pt-4 mb-8 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full
                group-even:ml-[18rem]'>
                    ...
                    <ul className='flex flex-wrap mt-4 gap-2'>{/*sm:mt-auto*/}
                        {tags.map((tag, index) => (
                            ...
                            upercase tracking-wider text-white rounded-full"key={index}> {tag} </li>
                        ))}
                    </ul>
                </div>

                <Image src={imageUrl} alt="Projects I worked on" quality={95} 
                className='absolute hidden sm:block top-8 -right-40 w-[28.25rem] rounded-t-lg shadow-2xl transition group-hover:scale-[1.0]
